...
function Dashboard() {
  ...
  // Show modal to add assignments back
  const [showHiddenAssignments, setShowHiddenAssignments] = React.useState(false)
  ...
  // Get hidden assignments list
  const hiddenAssignmentsList = React.useMemo(() => {
    if (!assignments) return []
    return assignments
  ...
  const unhideAssignment = (assignmentId: number) => {
    setHiddenAssignments(prev => {
      const next = new Set(prev)
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-full">
        ...
        {(pendingAssignments.length > 0 || hiddenAssignmentsList.length > 0) && (
          <div className="card p-5 bg-gradient-to-br from-orange-50 to-red-50 border-l-4 border-orange-500 flex flex-col" style={{height: currentCourse ? 'calc(100vh - 13rem)' : 'calc(100vh - 8rem)'}}>
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex items-center gap-2">
                {hiddenAssignmentsList.length > 0 && (
                  <button
                    onClick={() => setShowHiddenAssignments(!showHiddenAssignments)}
                    className="px-2 py-1.5 text-xs bg-white bg-opacity-50 hover:bg-opacity-100 rounded-lg transition-all duration-200 flex items-center gap-1 text-gray-600"
                    title={language === 'he' ? 'מטלות מוסתרות' : 'Hidden assignments'}
                  >
                    <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21" />
                    </svg>
                    {hiddenAssignmentsList.length}
                  </button>
                )}
                <Link
                  to="/assignments"
            ...
            {/* Hidden Assignments Modal/List */}
            {showHiddenAssignments && (
              <div className="mb-3 p-3 bg-white bg-opacity-50 rounded-lg border border-orange-100 animate-slide-down">
                <div className="flex items-center justify-between mb-2">
                  <h4 className="text-xs font-bold text-gray-700">
                    {language === 'he' ? 'מטלות מוסתרות' : 'Hidden Assignments'}
                  </h4>
                  <button
                    onClick={() => setShowHiddenAssignments(false)}
                ...
                <div className="max-h-32 overflow-y-auto space-y-1">
                  {hiddenAssignmentsList.map((assignment: any) => (
                    <div key={assignment.id} className="flex items-center justify-between p-2 bg-white rounded border border-gray-100 shadow-sm">
                      ...
                      <button
                        onClick={() => unhideAssignment(assignment.id)}
                        className="px-2 py-0.5 text-[10px] bg-blue-50 text-blue-600 rounded border border-blue-100 hover:bg-blue-100 transition-colors duration-200 whitespace-nowrap"
                      >
                        {language === 'he' ? 'החזר' : 'Restore'}
                      </button>
                    </div>
                  ))}
                </div>
            ...
            <div className="overflow-y-auto space-y-2 flex-1">
              {pendingAssignments.length === 0 ? (
                 <div className="flex flex-col items-center justify-center h-full text-center text-gray-500 py-8">
                   <div className="w-12 h-12 bg-white bg-opacity-50 rounded-full flex items-center justify-center mb-2">
                     <svg className="w-6 h-6 text-orange-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                       <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                     </svg>
                   </div>
                   <p className="text-sm">{language === 'he' ? 'אין מטלות ממתינות' : 'No pending assignments'}</p>
                   {hiddenAssignmentsList.length > 0 && (
                     <p className="text-xs text-gray-400 mt-1">
                       {language === 'he' ? `(ישנן ${hiddenAssignmentsList.length} מטלות מוסתרות)` : `(${hiddenAssignmentsList.length} hidden assignments)`}
                     </p>
                   )}
                 </div>
              ) : (
